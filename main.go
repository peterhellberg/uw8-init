...
const (
	...
	defaultBackupPath = "/run/user/1000/gvfs/smb-share:server=diskstation.local,share=backups/Code/Fantasy/MicroW8"
)
...
func parse(args []string, stderr io.Writer) (config, error) {
	...
	flags.StringVar(&cfg.title, "title", "", "The title of the MicroW8 cart")
...
func replacer(cfg config, name string, data []byte) []byte {
	switch name {
	case "Makefile":
		data = replaceOne(data, `TITLE="uw8-cart"`, `TITLE="`+cfg.title+`"`)
		data = replaceOne(data, `NAME=uw8-cart`, `NAME=`+cfg.dir)
